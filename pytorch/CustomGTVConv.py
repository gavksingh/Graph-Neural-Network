...
def custom_adj_weights(edge_index, edge_weight, num_nodes=None, flow="source_to_target", coeff=1.):
...
class CustomGTVConv(MessagePassing):
    r"""
    The custom GTVConv layer
    ...
    def forward(self, x: Tensor, edge_index: Adj, edge_weight: OptTensor = None, mask=None) -> Tensor:
        ...
        if isinstance(edge_index, Tensor) and edge_index.size(-1) == edge_index.size(-2):
            ...
            # Absolute differences between neighboring nodes
