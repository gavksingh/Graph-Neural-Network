...
from GTVConv import CustomGTVConv
...
# Model
class CustomClassificationModel(Model):
...
MP1 = [CustomGTVConv(mp_channels, delta_coeff=delta_coeff, activation=mp_activation, kernel_regularizer=L2(l2_reg_val))
       for _ in range(mp_layers)]

Pool1 = CustomAsymCheegerCutPool(int(N // 2), mlp_hidden=[mlp_hidden_channels for _ in range(mlp_hidden_layers)],
                                 mlp_activation=mlp_activation, totvar_coeff=totvar_coeff, balance_coeff=balance_coeff, kernel_regularizer=L2(l2_reg_val))

MP2 = [CustomGTVConv(mp_channels, delta_coeff=delta_coeff, activation=mp_activation, kernel_regularizer=L2(l2_reg_val))
       for _ in range(mp_layers)]

Pool2 = CustomAsymCheegerCutPool(int(N // 4), mlp_hidden=[mlp_hidden_channels for _ in range(mlp_hidden_layers)],
                                 mlp_activation=mlp_activation, totvar_coeff=totvar_coeff, balance_coeff=balance_coeff, kernel_regularizer=L2(l2_reg_val))

MP3 = [CustomGTVConv(mp_channels, delta_coeff=delta_coeff, activation=mp_activation, kernel_regularizer=L2(l2_reg_val))
       for _ in range(mp_layers)]

# Compile the model
model = CustomClassificationModel(
    n_out, mp1=MP1, pool1=Pool1, mp2=MP2, pool2=Pool2, mp3=MP3)
