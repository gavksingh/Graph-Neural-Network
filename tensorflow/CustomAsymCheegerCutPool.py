...
class CustomAsymCheegerCutPool(SRCPool):
    r"""
    A custom Asymmetric Cheeger Cut Pooling layer

    Args:
        k (int): Number of output nodes.
        mlp_hidden (list of int): Number of hidden units for each hidden layer in
            the MLP used to compute cluster assignments (if `None`, the MLP has only one output layer).
        mlp_activation (str): Activation for the MLP layers.
        return_selection (bool): Whether to return the selection matrix.
        use_bias (bool): Whether to use bias in the MLP.
        totvar_coeff (float): Coefficient for graph total variation loss component.
        balance_coeff (float): Coefficient for asymmetric norm loss component.
        kernel_initializer (str): Initializer for the weights of the MLP.
        bias_initializer (str): Initializer for the bias of the MLP.
